%% Data B: robust single-trial original-model fitting with fixed ranges
% This script implements Steps 2 and 3 of the single-trial workflow.
%
% Main features
...
%
% The input file must contain the structure DataB produced by
%   DataB_single_trial_preprocessing_Fz.m
%
% Required toolbox
...
inputFile = 'DataB_Fz_single_trial_preprocessed.mat';

outputMatFile = ...
    'DataB_Fz_single_trial_original_model_fit_fixed_ranges.mat';

outputTrialCsvFile = ...
    'DataB_Fz_single_trial_fit_summary_fixed_ranges.csv';

outputParameterCsvFile = ...
    'DataB_Fz_parameter_summary_by_cue_fixed_ranges.csv';
...
if ~isfile(inputFile)
    [selectedFile, selectedPath] = uigetfile( ...
        '*.mat', ...
        'Select DataB_Fz_single_trial_preprocessed.mat');
...
if ~isfield(loadedData, 'DataB')
    error('The selected MAT file does not contain a structure named DataB.');
end

DataB = loadedData.DataB;
validate_data_structure(DataB);

trials = collect_trials(DataB);
...
FitAnalysis = struct;
FitAnalysis.dataset = get_optional_field(DataB, 'dataset', 'Data B');
FitAnalysis.channel = get_optional_field(DataB, 'channel', 'Fz');
...
if ExportFigures
    ...
    exportgraphics(figRight, ...
        'DataB_right_single_trial_original_model_fits_fixed_ranges.pdf', ...
        'ContentType', 'vector', ...
        'BackgroundColor', 'white');

    exportgraphics(figLeft, ...
        'DataB_left_single_trial_original_model_fits_fixed_ranges.pdf', ...
        'ContentType', 'vector', ...
        'BackgroundColor', 'white');

    exportgraphics(figParameters, ...
        'DataB_parameter_distributions_by_cue_fixed_ranges.pdf', ...
        'ContentType', 'vector', ...
        'BackgroundColor', 'white');
end

%% =========================================================
%  Local function: validate the DataB structure
%% =========================================================

function validate_data_structure(DataB)

    requiredTopFields = {'right', 'left'};

    for i = 1:numel(requiredTopFields)
        if ~isfield(DataB, requiredTopFields{i})
            error('DataB is missing field: %s', requiredTopFields{i});
        end
    end
...
function trials = collect_trials(DataB)

    groupNames = {'right', 'left'};
    directionNames = {'Right', 'Left'};

    numberOfTrials = ...
        numel(DataB.right.trial_number) + ...
        numel(DataB.left.trial_number);
...
function figureHandle = plot_direction_fits( ...
    ...
    sgtitle(fitLayout, sprintf( ...
        'Data B %s-cue single-trial original-model fits', ...
        directionName));
end
...
function figureHandle = plot_parameter_distributions( ...
    ...
    sgtitle( ...
        'Data B parameter distributions: points, median, and IQR');
end
